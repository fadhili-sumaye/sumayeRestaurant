...
@Component
@Slf4j
public class HaloPesaProvider implements PaymentProvider {
    ...
    public boolean supports(Payment.PaymentProvider provider) {
        return Payment.PaymentProvider.HALOPESA.equals(provider);
    }
    ...
    public Payment.PaymentTransactionStatus processPayment(Payment payment) {
        log.info("Processing HaloPesa payment reference: {} for amount: {}", payment.getTransactionReference(), payment.getAmount());
